...
public class SysMenuServiceImpl implements ISysMenuService
{
    ...
    public List<SysMenu> selectMenuList(SysMenu menu, SysUser user) {
        if (SecurityUtils.isSuperAdmin()) { // super-admin获取所有超管菜单（super_flag=1）
        ...
        if (SecurityUtils.isComAdmin(user)) { //租户管理员，直接获取租户绑定的权限模版的菜单
     ...
     */
    @Override
    public Set<String> selectMenuPermsByTempId(String tempId)
    {
        List<String> perms = menuMapper.selectMenuPermsByTempId(tempId);
        Set<String> permsSet = new HashSet<>();
        for (String perm : perms)
        {
            if (StringUtils.isNotEmpty(perm))
